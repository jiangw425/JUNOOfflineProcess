...
bool GenCalibPDF::initialize()
{
    ...
    TFile* TestFile = new TFile(Form("ParaFile_%d.root",jobA), "RECREATE");
    ...
    datain.open("/cvmfs/juno.ihep.ac.cn/centos7_amd64_gcc830/Pre-Release/J21v1r0-Pre2/data/Simulation/ElecSim/Hamamatsu_pmtID.txt", std::ios::in);
    ...
    datain.open("/cvmfs/juno.ihep.ac.cn/centos7_amd64_gcc830/Pre-Release/J21v1r0-Pre2/data/Simulation/ElecSim/pmtdata.txt", std::ios::in);
...
void GenCalibPDF::GenPDF(const char *SavePath, TChain* calibChain, TChain* usercalibChain, int PlaceID, const double PositionX, const double PositionZ,
    ...
    for (int i=0;i<N;i++) {
        ...
        double r = sqrt(PMTX[i]*PMTX[i] + PMTY[i]*PMTY[i] + PMTZ[i]*PMTZ[i]);

        if(r>1.e-16) tmptheta = TMath::ACos(PMTZ[i]/r);
        if(TMath::Abs(PMTX[i])>1.e-16) {
            tmpphi = TMath::ATan(PMTY[i]/PMTX[i]);
            if(sgn(TMath::Sin(tmptheta)*TMath::Cos(tmpphi))!=sgn(PMTX[i])) tmpphi += TMath::Pi();
            if(tmpphi<0.)  tmpphi += 2.*TMath::Pi();
        }
...
double GenCalibPDF::sgn(double x)
{
