...
void ck_rec_totPE_Ge68_r_4()
{
    ...
    std::string sim_type[3] = {"detsim","elecsim","calib"};
    TString tree_name[3] = {"evt","eventindex","calibevt"};
    for(int t=0;t<nc;++t) {//if(t!=2) continue;
    for(int i=0;i<CalibPosNum;i++) {
        TString tmp_path = Form("%s_"+StrPosX->at(i)+"_"+StrPosY->at(i)+"_"+StrPosZ->at(i), Ge68_path[t].c_str());
        TString FilePath[3];
        TChain *tc[3];
        for(int s=0;s<3;s++){
            FilePath[s] = Form(tmp_path+"/%s/user-root/user-%s-*",sim_type[s].c_str(), sim_type[s].c_str());
            // cout << FilePath[s] << endl;
            tc[s] = new TChain(tree_name[s]);
            tc[s]->Add(FilePath[s]);
        }
        // cout << i << "\t" << positionZ[1][i] <<"\t" <<  tc[0]->GetEntries() << "\t" <<  tc[1]->GetEntries() <<"\t"<<  tc[2]->GetEntries()<<endl;
        if(tc[1]->GetEntries() != tc[2]->GetEntries()){ cout << "Wrong!" << endl; return;}

        tc[0]->SetBranchStatus("*",0);
        tc[1]->SetBranchStatus("*",0);
        tc[2]->SetBranchStatus("*",0);

        tc[0]->SetBranchStatus("edep",1);
        tc[1]->SetBranchStatus("tags",1);
        tc[1]->SetBranchStatus("entries",1);
        tc[2]->SetBranchStatus("TotalPE",1);

        float edep;
        vector<string> *tags = new vector<string>;
        vector<int> *entries = new vector<int>;
        float rec_totPE;
        tc[0]->SetBranchAddress("edep",&edep);
        tc[1]->SetBranchAddress("tags",&tags);
        tc[1]->SetBranchAddress("entries",&entries);
        tc[2]->SetBranchAddress("TotalPE",&rec_totPE);

        TH1F *nPE_pdf = new TH1F("rec_totPE","rec_totPE",600,800,3200);

        for(int elec_en=0;elec_en<tc[1]->GetEntries();++elec_en){
            tags->clear();
            entries->clear();

            tc[1]->GetEntry(elec_en);
            int sim_en = entries->at(0);
            // vector<string>::iterator tags_iter = tags->begin();
            // vector<int>::iterator entries_iter = entries->begin();
            // while(iter!=tags->end()){
                // if(*iter=="eventinput") sim_en = *entries_iter;
                // tags_iter++;
                // entries_iter++;
            // }
            for(int nt=0;nt<tags->size();nt++)  if(tags->at(nt)=="eventinput") sim_en = entries->at(nt);
            
            tc[0]->GetEntry(sim_en);
            if(edep<1.0215||edep>1.0225) continue;
            tc[2]->GetEntry(elec_en);
            nPE_pdf->Fill(rec_totPE);
        }
        ...
        cout << i <<"\t" << positionZ[1][i] <<"\tavg_nPE:\t"<< totPE[t][i] << "\tmean:\t" << mean << "\tres:\t" << totPEres[t][i] << "\tentries:\t" << tc[2]->GetEntries() <<endl;
        
        // TCanvas *tmpc = new TCanvas("tmpc","tmpc",1600,900);
        // tmpc->cd();
        // nPE_pdf->Draw();
        // tmpc->Print(Form("tmp_%d.png",i));
        // tmpc->~TCanvas();

        for(int s=0;s<3;s++) tc[s]->~TChain();
    }}
    // for(int i=0;i<CalibPosNum;i++){
