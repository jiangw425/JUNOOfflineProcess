...
void filterWaves()
{
  ...
  for (int i = start; i < end; i++) {
    ...
    if (hmmtpmt[id] == true) {
      ...
      k = 0;
      for (int j = adcl-wf_start; j < adcl; j++) arr[k++] = w->GetBinContent(j + 1);
      for (int j = 0; j < adcl-wf_start; j++) {
        arr[k++] = w->GetBinContent(j + 1);
        w->SetBinContent(j + 1, arr[j]);
      }
      for (int j = adcl-wf_start; j < adcl; j++) w->SetBinContent(j + 1, arr[j]);
    } else {
      // wd->set_input(w_tmp, h_filter0, spere[id], speim[id]);
