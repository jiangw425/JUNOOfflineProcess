...
void En_NL() {
    ...
    for(int  ci=0;ci<cNum;ci++) {
        ...
        for(int kNum=0;kNum<eNum;kNum++) {
            ...
            if(kNum==0) energyscale = 1.022/mean;
            // if(ci==0) {
            ...
            ojw << TrueEnergy[kNum] << '\t' << mean/TrueEnergy[kNum] << endl;
        ...
        TCanvas *cjw = new TCanvas("a","a",1600,900);
        TGraph *gjw = new TGraph(eNum,TrueEnergy,jw_nl);
        gjw->GetXaxis()->SetTitle("True Energy/MeV");
        gjw->GetYaxis()->SetTitle("E_{rec}/E_{true}");
        cjw->cd();
        gjw->Draw("AC*");
        cjw->Print("nl.png");
        ojw.close();
        return;
        ofstream ojw("kRes_eff.txt");
        std::cout << std::endl;
        ...
        ojw << sqrt(a*a + b*b + c*c) << "\t" << sqrt(a*a + b*b*1.6*1.6 + c*c/1.6/1.6) << std::endl;
        ojw.close();
        return;
